...
export default function Dashboard() {
  ...
  const stats = {
    totalFiles: pcapList?.length || 0,
    totalPackets: pcapList?.reduce((sum: number, p: { packet_count: number }) => sum + (p.packet_count || 0), 0) || 0,
    totalBytes: pcapList?.reduce((sum: number, p: { file_size: number }) => sum + (p.file_size || 0), 0) || 0,
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-card border rounded-lg p-6">
        ...
        {pcapList && pcapList.length > 0 ? (
          <div className="space-y-2">
            {pcapList.slice(0, 5).map((pcap: { id: number; filename: string; file_size: number; packet_count: number; created_at: string }) => (
              <Link
                key={pcap.id}
                to={`/pcap/${pcap.id}`}
      ...
      {alerts && alerts.length > 0 && (
        <div className="bg-card border rounded-lg p-6">
          ...
          <div className="space-y-2">
            {alerts.slice(0, 5).map((alert: { id: number; title: string; severity: string; created_at: string }) => (
              <div
                key={alert.id}
