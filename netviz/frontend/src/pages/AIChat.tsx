...
export default function AIChat() {
  ...
  const summaryMutation = useMutation({
    mutationFn: () => aiApi.getSummary(selectedPcap!, provider),
    onSuccess: (res) => {
      setMessages((prev) => [
        ...prev,
  ...
  const analyzeMutation = useMutation({
    mutationFn: (type: string) => aiApi.analyze(selectedPcap!, type, provider),
    onSuccess: (res) => {
      setMessages((prev) => [
        ...prev,
      ...
      <div className="p-4 border-b flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          >
            ...
            {pcapList?.map((p: { id: number; filename: string }) => (
