                  ...
                  <tbody>
                    {topTalkers.map((talker) => (
                      <tr key={talker.ip} className="border-b">
                        ...
                        <td className="text-right p-2">{formatNumber(talker.packets_sent + talker.packets_received)}</td>
                        <td className="text-right p-2">{formatBytes(talker.bytes_sent + talker.bytes_received)}</td>
        ...
        <tbody>
          {packets?.map((pkt) => (
            <tr key={pkt.id} className="border-b hover:bg-accent/50">
              ...
              <td className="p-2 font-mono">{pkt.src_ip}{pkt.src_port ? `:${pkt.src_port}` : ''}</td>
              <td className="p-2 font-mono">{pkt.dst_ip}{pkt.dst_port ? `:${pkt.dst_port}` : ''}</td>
        ...
        <tbody>
          {connections?.map((conn) => (
            <tr key={conn.id} className="border-b hover:bg-accent/50">
              <td className="p-2 font-mono">{conn.src_ip}{conn.src_port ? `:${conn.src_port}` : ''}</td>
              <td className="p-2 font-mono">{conn.dst_ip}{conn.dst_port ? `:${conn.dst_port}` : ''}</td>
              <td className="p-2">{conn.protocol}</td>
              <td className="p-2 text-right">{conn.packet_count}</td>
              <td className="p-2 text-right">{formatBytes(conn.src_to_dst_bytes)}</td>
              <td className="p-2 text-right">{formatBytes(conn.dst_to_src_bytes)}</td>
            </tr>
          ))}
        </tbody>
      </table>
